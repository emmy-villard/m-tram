...
The daily refresh also builds two-hour aggregates from the freshly rebuilt hourly aggregate table. This second layer avoids recalculating joins against the raw source tables while making predefined dashboard time blocks fast to query.

The two-hour table uses the selected block and the traffic type as aggregation dimensions. The DAG rebuilds it immediately after the hourly table has been rebuilt.
...
- A **manual full-refresh DAG** rebuilds the complete hourly and two-hour aggregates from all available raw source data. It is intended for initial population, source corrections, backfills, and changes to the aggregation logic.
...
4. Replace the affected hourly records with the rebuilt records, then rebuild and replace the affected two-hour records.
...
The manual full refresh validates the complete rebuilt result, then replaces the contents of both aggregate tables with the newly computed history.
...
If historical source data outside the 24-hour window is corrected, backfilled, deleted, or reprocessed, the incremental refresh is not sufficient. The manual full-refresh DAG must be run to rebuild both aggregate tables from the complete raw history.

## Querying Strategy

FastAPI endpoints query the hourly aggregate dataset using the requested date range, traffic type, weekday, environmental metric, and optional pollutant subtype. Two-hour requests query the dedicated two-hour aggregate table.
